<think>
The error `KeyError: 'embedding_model_name'` occurs because the initial `config` loaded in `create_gradio_app` does not contain the key "embedding_model_name". This happens if the `config.json` file doesn't already have this key-value pair, and the code relies on `gr.State(config["embedding_model_name"])` *before* the user has a chance to save it via the settings. The `load_config` function only loads from the default config if the `config.json` is completely missing, not if a key inside `config.json` is absent.

Here's the breakdown of the problem and the solution:

**Problem:**

1.  **Initial Load:** When the application first starts, `create_gradio_app` loads the configuration from `config.json`.
2.  **Missing Key:** If `config.json` exists but *doesn't* contain the `embedding_model_name` key (e.g., the first time the user runs the updated code or if they manually deleted the key from the file), the `config["embedding_model_name"]` access will raise a `KeyError`.
3.  **Gradio State:** The `gr.State` in the `index_button.click` event tries to access this potentially missing key *before* the user has any chance to set it in the "Settings" tab.

**Solution:**

There are two main approaches to fix this, and I will combine them for the most robust solution:

1.  **Default in `load_config`:** Modify the `load_config` function to not only use the `DEFAULT_CONFIG` when the file is missing, but to also *update* the loaded configuration with any missing keys from `DEFAULT_CONFIG`. This ensures that all required keys are present, even if the `config.json` is an older version.

2.  **Handle in `create_gradio_app`:** As an extra precaution, even with the improved `load_config`, it's good practice to handle the potential absence of the key within `create_gradio_app` itself. Use `.get()` with a default value when accessing `config["embedding_model_name"]`. This provides an immediate fallback if, for some reason, the key is still missing after the `load_config` step.

**Implementation:**

Here's the modified `load_config` and the relevant part of `create_gradio_app`:
...
def create_gradio_app() -> gr.Blocks:
    ...
    # ... (rest of the Gradio UI definition)
...
*   **`load_config`:** The updated `load_config` now iterates through the `DEFAULT_CONFIG` and adds any missing keys to the `config` loaded from the file. This ensures that `config` always has all the necessary keys.
*   **`create_gradio_app`:** Inside the `index_button.click` call, I replaced `config["api_key"]` with `config.get("api_key", DEFAULT_CONFIG["api_key"])`, and similarly for `api_base_url` and `embedding_model_name`.  The `.get()` method attempts to retrieve the value associated with the key. If the key is *not* found, it returns the provided default value (in this case, the corresponding value from `DEFAULT_CONFIG`). This prevents the `KeyError`.

With these changes, the application will now gracefully handle the case where `embedding_model_name` (or `api_key`, `api_base_url`) is missing from `config.json` on the initial load, preventing the crash. The app will use the default value until the user explicitly sets it in the Settings tab and saves the configuration.
Complete code:
```python
...
The code provided is the complete and corrected code, with the fix for the KeyError. The previous turn explained the reasoning and implementation of the fix. No further code is required here.
